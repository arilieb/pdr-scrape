...
class Example(Frame):
    ...
    def initUI(self):
        ...
        self.txt.insert('end',
            "-Instructions-\n\nPlease input your host name, username, and password, then click submit to allow for database access\n\nStart - Constructs database pdr_db with tables drug_index and drug_interactions (if not already constructed), then begins scraping PDR.net for drug interactions. This process will likely take several hours to complete, and will continue until it has finished or the Stop button is pressed. Process DOES NOT terminate on window closure.\n\nStop - terminates scraping of PDR.net. Please note that pressing start, then stop, then start again, will restart scraping process.\n\nQuery - allows for database queries specific to pdr_db, ie SELECT * FROM drug_index"
            )
