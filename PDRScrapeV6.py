...
"""further simplification of PDRScrape-specific SQL commands"""
...
from tkinter import (Tk, BOTH, E, W, S, N, END, NORMAL, DISABLED)
...
def PDR_Scrape():
    ...
    #Sets up database and tables if they do not already exist
    cmd.makeDB()
    cmd.makeDrugIndexTable()
    cmd.makeDrugInteractionsTable()
    
    #Requests pdr.net 'Browse by Drug Name' landing page.
    page = requests.get(home)
    soup = BeautifulSoup(page.content, 'html.parser')
    
    navLinks = [] #Container for alphabetical drug index landing pages 
    ...
    for link in soup.find_all('a', href=True):
        ...
        if 'browse-by-drug-name?' in link.get('href') and link.get('href')[-1].isalpha():
            #Fills navLinks with links to alphabetical drug index landing pages.  
            navLinks.append(link.get('href'))
    
    #Iterates through alphabetical drug index landing pages
    for i in navLinks:
        ...
        while True:
            ...
            #progressive wait to account for DDoSing exceptions 
            page = ''
            wait = 1
            while page == '':
                try:
                    #Requests numerical sub-page 
            ...
            soup = BeautifulSoup(page.content, 'html.parser') #Parses numerical sub-page
            ...
            #Matches brand names to summary links, constructs chemical names 
            for j in range(0,len(drugLinks)):
                for k in range (0,len(drugNameSoup)):
                    ...
                    bnFirstWord = bn.split(' ', 1)[0].replace('/', '-').replace(',', '-').replace('.','-').replace('+','-').replace('•', 'bull').replace('%', '-').lower() #First word of brand name
                    
                    #Checks if summary link matches brand name
                    if bnFirstWord in drugLinks[j]:
                        #Cleans chemical name associated with brand name from drugNameSoup
            ...
            #Constructs database insert statements.
            for name, chem, link in zip(brandNames, chemNames, drugLinks):
                try:
                    #Attempts to insert to drug_index
                    cmd.insertToDrugIndex(name, chem, link) 
                except Exception as e:
                    #Passes if exception is due to entry already existing in table 
                    if len(cmd.sql.tc('pdr_db', 'SELECT * FROM drug_index WHERE brand_name = "' + name +'"')) > 0:
                        pass
                    else:
                        #Prints error statement and location of error 
                        print("Something has gone wrong inserting \n\"{}\", \n\"{}\", \n\"{}\" \nto drug_index table".format(name,chem,link))
                        print(e)
                
                hdr = {'User-Agent':'Mozilla/5.0'} #Used to disguise HTML request in order to read JS dynamically loaded text
                ...
                while page == '':
                    try:
                        #Disguised requests for drug summary page with dynamically loaded JS text 
                ...
                #Parses drug summary page
                soup = BeautifulSoup(page.content, 'html.parser')
                tags = soup.find_all('div', 'clearfix drugSection')
                
                #Constructs drug_interactions insert statements
                for x in tags:
                    #Finds drug interactions among other summary page sections
                    if x.find('h3', 'drugSummary').text == 'DRUG INTERACTIONS':
                        iNames = x.find_all('strong') #Interaction names all tagged with 'strong'
                        iDescriptions = x.text.split(': (') #List currently containing interaction strength and description 
                        
                        #Deletes section title from description list
                        del iDescriptions[0]
                        
                        #Matches interaction names to descriptions, cleans interaction strength from description
